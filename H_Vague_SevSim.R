...
Obs_BF_HV <- BF_Comp_Brute_Force(20,5,15,c(0.1,0.5),c(0.5,0.9))
Obs_BF_Error <- BF_Comp_Brute_Force(20,5,15,c(0.25,0.85),c(0.15,0.75)) # SEV against c(0.25,0.75) was around 0.74
Obs_BF_Test <- Obs_BF_HV / Obs_BF_Error

BF_HVague_Sampling <- rep(NA,NSim)
BF_HConstruct_Sampling <- rep(NA,NSim)
BF_HError_Sampling <- rep(NA,NSim)

trueThetaA_Range <- c(0.25,0.85)
trueThetaB_Range <- c(0.15,0.75)
...
for (i in 1:NSim) {
  ...
  propTestA <- prop.test(tempY_A,NPerGroup,conf.level=0.95)
  propTestB <- prop.test(tempY_B,NPerGroup,conf.level=0.95)
...
HVagueSim <- list(Obs_BF_HV = Obs_BF_HV,
                  ...
                  Error_Hypothesis = list(A = c(0.25,0.85),B=c(0.15,0.75)))
saveRDS(HVagueSim,file = "simulation_output/HVagueSim.RDS")
